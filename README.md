...
| `fbrain ask <query> [-n N \| --limit N] [--no-llm] [--explain] [--type T]…` | LLM-expanded hybrid retrieval: BM25 + vector fused via Reciprocal Rank Fusion. Wider recall than `search` — paraphrase via vector, rare-token / acronym matches via BM25. Repeatable `--type` narrows both the BM25 corpus and the vector schemas filter. `-n` and `--limit` are aliases |
