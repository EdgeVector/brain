...
| `fbrain_append` | `slug` + `chunk?`/`chunk_path?`/`chunk_b64?` + `type?` + `raw?` | Append a chunk to a record's body without a full rewrite. `text` is accepted as an alias for `chunk` (the param agents naturally reach for). Reads the full body server-side (no get-window truncation) and can only GROW it, so it never trips the shrink guard. Without `type`, errors on ambiguous slug |
