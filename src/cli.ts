...
async function runList(args: Argv, verbose: Verbose): Promise<number> {
  ...
  // Validate `--updated-since` BEFORE readConfig so a malformed value exits 2
  // (usage) rather than being masked by a config-missing exit-1 — same
  // pre-config ordering as the integer-flag guards. Peek the raw token (it
  // may follow `--updated-since` or `--updated-since=...`) and run it through
  // the shared parser for its throw side effect; the value is re-parsed below.
  {
    const rawSince = peekNextValue(args, "--updated-since");
    if (rawSince !== undefined) parseUpdatedSince(rawSince);
  }
  let parsed;
  try {
    parsed = parseCommandArgs(
  ...
  // `--offset` is pre-validated non-negative; parse it here. `parseInt("0")`
  // is 0 (falsy), so guard on the raw string presence, not truthiness of the
  // parsed number, or `--offset 0` would be dropped.
  const offset =
