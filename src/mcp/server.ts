...
export function createFbrainMcpServer(opts: CreateServerOptions): McpServer {
  ...
  server.registerTool(
    "fbrain_list",
    {
      ...
      // `fbrain_get` for the full record) or `{ count: N }` (count mode). Both
      // keys are optional so one output schema covers both modes; exactly one
      // is present per call, matching the `--count` split of the CLI shape.
      outputSchema: {
        records: z
          .array(summarySchema)
          .optional()
      ...
      runReadTool<ListResult>(
        ...
        (cfg, print, onResult) => {
          ...
          if (args.updated_since !== undefined) {
            // Same parser the CLI `--updated-since` flag uses — single source
            // of truth for ISO/relative token handling; a bad value throws a
            // usage FbrainError surfaced to the agent as an error result.
            listOpts.updatedSinceMs = parseUpdatedSince(args.updated_since);
          }
          return listCmd(listOpts);
        },
        // Single-sourced with the CLI `--json` shape: mirror the discriminated
        // `ListResult` into the MCP object — `{ records }` in row mode,
        // `{ count }` in count mode — so structuredContent can't drift.
        (result) =>
          result.mode === "count"
            ? { count: result.count }
