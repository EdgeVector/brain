...
// every signature check the client performs.
//
// The 12 golden vectors that pin cross-implementation agreement live at
// `fold/app_identity_crypto/tests/golden_vectors.rs`; `test/unit/jcs.test.ts`
...
// JSON value subset JCS operates over. `serde_json::Value` has the same shape.
...
function serializeArray(arr: JsonValue[]): string {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    const el = arr[i];
    parts[i] = serialize(el as JsonValue);
  }
  return "[" + parts.join(",") + "]";
}
...
function serializeObject(obj: { [key: string]: JsonValue }): string {
  ...
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i]!;
    parts[i] = serializeString(key) + ":" + serialize(obj[key] as JsonValue);
  }
  return "{" + parts.join(",") + "}";
}
