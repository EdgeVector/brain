...
import { buildReindexFields, pickRepairedTitle, reindexCmd } from "../../src/commands/reindex.ts";
...
// One-shot repair for records whose title is the literal text of a YAML
// block-scalar indicator (the brain-corruption case from #7852b).
describe("pickRepairedTitle", () => {
  test("falls back to the first H1 in the body", () => {
    const r = pickRepairedTitle("# The Real Title\n\nbody text", "the-slug");
    expect(r).toEqual({ title: "The Real Title", source: "h1" });
  });

  test("skips leading blank lines before the H1", () => {
    const r = pickRepairedTitle("\n\n# Real Title\nbody", "the-slug");
    expect(r).toEqual({ title: "Real Title", source: "h1" });
  });

  test("only the leading H1 counts; a later H1 does not", () => {
    const r = pickRepairedTitle("not a heading\n# Later H1", "the-slug");
    expect(r).toEqual({ title: "the-slug", source: "slug" });
  });

  test("no H1 → slug fallback", () => {
    const r = pickRepairedTitle("just body text\nno heading at all", "fallback-slug");
    expect(r).toEqual({ title: "fallback-slug", source: "slug" });
  });

  test("empty body → slug fallback", () => {
    const r = pickRepairedTitle("", "my-slug");
    expect(r).toEqual({ title: "my-slug", source: "slug" });
  });
});

describe("reindexCmd --repair-titles", () => {
  test("repairs only records whose title is a literal block-scalar indicator", async () => {
    const { restore, mutations } = stubFetch({
      queries: {
        [TEST_HASHES.design]: [
          // Corrupt: literal `>-` as title; body has an H1.
          designRow("p-foo", { title: ">-", body: "# Foo Project\n\ndescription" }),
          // Healthy: normal title — should NOT be touched.
          designRow("p-bar", { title: "P-Bar Real Title", body: "body" }),
          // Tombstoned: ignored entirely.
          designRow("p-dead", { title: ">-", tags: [TOMBSTONE_TAG], body: "" }),
        ],
        [TEST_HASHES.concept]: [
          // Corrupt: `|` indicator, body has no H1 → falls back to slug.
          conceptRow("c-bad", { title: "|", body: "no h1 here" }),
        ],
      },
    });
    try {
      ...
      // Only the two corrupt records were updated; healthy and tombstoned
      // were skipped.
      ...
      // Audit log lists every change with the source signal.
      const joined = lines.join("\n");
      expect(joined).toContain("repaired 2 title(s)");
      expect(joined).toContain("design/p-foo");
      expect(joined).toContain('"Foo Project"');
      expect(joined).toContain("concept/c-bad");
    } finally {
      restore();
    }
  });

  test("--repair-titles --dry-run reports without writing", async () => {
    const { restore, mutations } = stubFetch({
      queries: {
        [TEST_HASHES.design]: [
          designRow("p-foo", { title: ">-", body: "# Real Title" }),
        ],
      },
    });
    try {
      ...
      expect(lines.join("\n")).toContain("dry-run: would repair 1");
    } finally {
      restore();
    }
  });

  test("idempotent: a second --repair-titles run finds nothing to fix", async () => {
    // After a repair, titles are clean. A re-run sees only healthy rows.
    const { restore, mutations } = stubFetch({
      queries: {
        [TEST_HASHES.design]: [designRow("p-foo", { title: "Real Title" })],
      },
    });
    try {
      const lines: string[] = [];
      ...
      expect(lines.join("\n")).toContain("repaired 0 title(s)");
    } finally {
      restore();
    }
  });

  test("recognizes every block-scalar indicator (>, >-, >+, |, |-, |+)", async () => {
    const indicators = [">", ">-", ">+", "|", "|-", "|+"];
    const { restore, mutations } = stubFetch({
      queries: {
        [TEST_HASHES.design]: indicators.map((ind, i) =>
          designRow(`d-${i}`, { title: ind, body: `# Body ${i}` }),
        ),
      },
    });
    try {
      ...
      expect(result.repaired!).toHaveLength(indicators.length);
      expect(mutations.length).toBe(indicators.length);
    } finally {
      restore();
    }
  });

  test("--repair-titles --type narrows scope", async () => {
    const { restore, mutations } = stubFetch({
      queries: {
        [TEST_HASHES.design]: [designRow("d1", { title: ">-", body: "# A" })],
        [TEST_HASHES.task]: [taskRow("t1", { title: ">-", body: "# B" })],
      },
    });
    try {
      const result = await reindexCmd({
      ...
      expect(result.repaired!).toHaveLength(1);
      expect(result.repaired![0]!.type).toBe("task");
      expect(mutations.length).toBe(1);
    } finally {
      restore();
    }
